...
CAU_MIN_LENGTH = 5 # Độ dài tối thiểu của cầu để được nhận diện (5-7 là hợp lý)
...
GAME_CONFIGS = {
    'Luckywin': {
        # ### CẬP NHẬT API MỚI NHẤT BẠN CUNG CẤP ###
        'api_url': 'https://1.bot/GetNewLottery/LT_Taixiu',
        'display_name': 'Luckywin 🎲'
    }
}

# Lưu trữ các mẫu cầu đã học (trong bộ nhớ)
...
def send_telegram_message(chat_id, text, parse_mode='MarkdownV2', disable_web_page_preview=True):
    ...
    try:
        ...
        print(f"Payload lỗi: {json.dumps(payload, ensure_ascii=False)}")
    ...
    if len(history) >= CAU_MIN_LENGTH:
        ...
        if is_zigzag:
            ...
            if current_zigzag_length >= CAU_MIN_LENGTH:
                prediction = 'X' if history[-1] == 'T' else 'T'
                return {
...
def process_game_api_fetch(game_name, config):
    try:
        ...
        # ### CẬP NHẬT LOGIC XỬ LÝ DỮ LIỆU TỪ API CỦA BẠN ###
        # Dựa trên API bạn cung cấp, full_api_data có thể trực tiếp là dict chứa thông tin phiên
        latest_game_data_raw = full_api_data 
        
        if not latest_game_data_raw or not isinstance(latest_game_data_raw, dict):
            print(f"LỖI: Dữ liệu từ API {game_name} không đúng cấu trúc mong đợi (không phải dict hoặc trống): {full_api_data}")
            return

        phien = str(latest_game_data_raw.get('Expect')) # Lấy từ trường 'Expect'
        open_code_str = str(latest_game_data_raw.get('OpenCode')) # Lấy từ trường 'OpenCode'
        ...
        if phien and open_code_str:
            dice_values = []
            try:
                # Phân tách chuỗi "4,5,2" thành các số nguyên
                dice_values_str = [d.strip() for d in open_code_str.split(',') if d.strip()]
                for d_str in dice_values_str:
                    if d_str.isdigit():
                        dice_values.append(int(d_str))
                    else:
                        print(f"CẢNH BÁO: Giá trị xúc xắc không phải số: '{d_str}' trong OpenCode '{open_code_str}'")
            ...
            tong = sum(dice_values) if dice_values else None
            ...
            if tong is not None:
                if tong >= 11:
                    ...
                    ket_qua = 'X'
                # Chưa xử lý Bão, bạn có thể thêm logic ở đây nếu muốn bot dự đoán cả Bão.
            ...
            print(f"LỖI: Dữ liệu từ API {game_name} không đầy đủ (thiếu Expect hoặc OpenCode): {latest_game_data_raw}")
...
def learn_new_patterns(history, game_name):
    ...
    for result_type in ['T', 'X']:
        for length in range(1, len(history)):
            if len(history) - length -1 < 0:
                continue
            pattern_segment = history[len(history) - length - 1 : len(history) - 1]
            next_result = history[-1]

            if not pattern_segment:
                continue

            if all(r == pattern_segment[0] for r in pattern_segment):
                pattern_string = "".join(pattern_segment)
                if next_result == pattern_segment[0]:
    ...
    for length in range(1, len(history)):
        ...
        is_zigzag = True
        if len(pattern_segment) > 1:
            for i in range(len(pattern_segment) - 1):
                if pattern_segment[i] == pattern_segment[i+1]:
                    is_zigzag = False
                    break
        else:
            is_zigzag = False
    ...
    for pattern_string, count in new_cau_dep.items():
        if game_name in LEARNED_PATTERNS:
...
# --- CHỨC NĂNG NHẬP LỊCH SỬ TỪ FILE ---
                ...
                for line in f:
                    ...
                    if game_name and line.startswith("Phiên:"):
                        ...
                        if match:
                            ...
                            xuc_xac_1 = int(xuc_xac_parts_raw[0]) if len(xuc_xac_parts_raw) > 0 and xuc_xac_parts_raw[0].isdigit() else None
